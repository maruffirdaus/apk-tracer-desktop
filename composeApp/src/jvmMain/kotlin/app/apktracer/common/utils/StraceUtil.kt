...
class StraceUtil() {
    suspend fun tracePackage(packageName: String, outputDir: String, timeout: Int = 30): Boolean =
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val pid =
                            findProcessId(packageName)
                                    ?: throw IllegalStateException(
                                            "Could not find process ID for package: $packageName"
                                    )
                    ...
                    val outputFile =
                            PlatformFile(
                                    PlatformFile(outputDir).also { it.file.mkdirs() },
    ...
    private suspend fun findProcessId(packageName: String): Int? =
            withContext(Dispatchers.IO) {
                try {
                    val process =
                            ProcessBuilder("adb", "shell", "su", "-c", "ps -A | grep $packageName")
                                    .start()
                    process.waitFor()
    ...
    private suspend fun pushStraceBinary() =
            withContext(Dispatchers.IO) {
                val binaryFile = PlatformFile(FileKit.filesDir, "binaries/strace-arm64")

                if (!binaryFile.exists()) {
                    extractStraceBinary()
                }

                ProcessBuilder("adb", "push", binaryFile.absolutePath(), "/data/local/tmp/strace")
                        .inheritIO()
                        .start()
                        .waitFor()

                ProcessBuilder("adb", "shell", "su", "-c", "chmod 755 /data/local/tmp/strace")
                        .inheritIO()
                        .start()
                        .waitFor()
            }

    private suspend fun extractStraceBinary() =
            withContext(Dispatchers.IO) {
                val binaryName = "strace-arm64"
                ...
                inputStream.use { input ->
                    ...
                    binaryFile.file.outputStream().use { output -> input.copyTo(output) }
                }
            }
}
