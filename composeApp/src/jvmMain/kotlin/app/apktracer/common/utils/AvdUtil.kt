...
class AvdUtil() {
    suspend fun duplicate(avdIni: String): String =
            withContext(Dispatchers.IO) {
                ...
                file.parent()?.let {
                    val duplicatedFile = PlatformFile(it, "${file.nameWithoutExtension}_Copy.ini")
    ...
    suspend fun start(avdIni: String) =
            withContext(Dispatchers.IO) {
                ProcessBuilder(
                                "/Users/wildanniam/Library/Android/sdk/emulator/emulator",
                                "-avd",
    ...
    suspend fun kill() =
            withContext(Dispatchers.IO) {
                val process = ProcessBuilder("adb", "emu", "kill").start()
                process.waitFor()
            }

    suspend fun delete(avdIni: String) =
            withContext(Dispatchers.IO) {
                val file = PlatformFile(avdIni)
                val lines = file.readString().lines().toMutableList()
                for (line in lines) {
